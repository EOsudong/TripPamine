...
function getCurrentLocationOrThrow(): Promise<MysteryQuestCompleteRequest> {
  return new Promise((resolve, reject) => {
    if (!window.isSecureContext) {
      reject(
          new Error("GPS는 HTTPS 주소 또는 localhost에서만 사용할 수 있습니다."),
      );
      return;
    }
...
export default function MysteryTourPlay() {
  ...
  const [processingAction, setProcessingAction] = useState<
      "complete" | "skip" | "abandon" | null
  >(null);
  const [tourResult, setTourResult] = useState<
      "SUCCESS" | "WITH_SKIP" | null
  >(null);
  ...
  const handleCompleteQuest = async () => {
    if (!quest || !mysteryTourId || isProcessing) return;
    ...
    try {
      ...
      const location =
          quest.verifyType === "GPS"
              ? await getCurrentLocationOrThrow()
              : undefined;
  ...
  const handleSkipQuest = async () => {
    ...
    try {
      ...
      const nextQuest = await skipMysteryQuest(
          Number(mysteryTourId),
          quest.mysteryQuestId,
      );
      ...
      if (nextQuest) {
        ...
        setQuest(null);
        setTourResult("WITH_SKIP");
      }
    } catch (error) {
      console.error("퀘스트 스킵 실패:", error);
      setErrorMessage(
          extractQuestErrorMessage(error, "퀘스트를 스킵하지 못했습니다."),
      );
    } finally {
      setProcessingAction(null);
    }
  ...
  const handleAbandonTour = async () => {
    if (!mysteryTourId || isProcessing) return;

    const confirmed = window.confirm(
        "미스터리 투어 전체를 포기하시겠습니까?\n\n남은 퀘스트는 모두 종료되며 이 투어는 다시 진행할 수 없습니다.",
    );
    ...
    try {
      ...
      setErrorMessage(null);
      await abandonMysteryTour(Number(mysteryTourId));
      alert("미스터리 투어를 포기했습니다. 새로운 투어를 신청할 수 있습니다.");
      navigate("/", { replace: true });
    } catch (error) {
      console.error("미스터리 투어 포기 실패:", error);
      setErrorMessage(
          extractQuestErrorMessage(error, "미스터리 투어를 포기하지 못했습니다."),
      );
      setProcessingAction(null);
    }
  };
              ...
              <div className="mt-10 relative overflow-hidden rounded-3xl border border-violet-500/30 bg-gradient-to-br from-slate-900 to-indigo-950 p-8 shadow-2xl">
                ...
                <span className="px-3 py-1 rounded-full bg-violet-500/20 border border-violet-400/20 text-violet-300 text-[11px] font-black tracking-widest">
                  ...
                  <button
                      ...
                      className="mt-6 w-full py-4 rounded-2xl bg-gradient-to-r from-violet-500 to-indigo-500 text-white font-black text-sm hover:scale-[1.01] transition-all disabled:cursor-not-allowed disabled:opacity-50"
                  >
                  ...
                  <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <button
                        ...
                        disabled={isProcessing}
                        className="w-full py-3 rounded-2xl border border-amber-400/30 bg-amber-400/5 text-amber-200 font-bold text-xs hover:bg-amber-400/10 transition-all disabled:cursor-not-allowed disabled:opacity-50"
                    >
                    ...
                    <button
                        ...
                        disabled={isProcessing}
                        className="w-full py-3 rounded-2xl border border-rose-500/30 bg-rose-500/5 text-rose-300 font-bold text-xs hover:bg-rose-500/10 transition-all disabled:cursor-not-allowed disabled:opacity-50"
                    >
                      {processingAction === "abandon"
                          ? "⏳ 포기 처리 중..."
                          : "🚫 투어 전체 포기"}
                    </button>
                  </div>
