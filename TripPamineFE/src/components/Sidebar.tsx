...
const navItems: NavGroup[] = [
  {
    ...
    items: [
      ...
      { icon: "🗺️", label: "여행지 탐색", to: "/#destinations" },
      { icon: "🎉", label: "축제 & 행사", to: "/#festivals" },
    ],
  },
  {
    group: "마이",
...
export default function Sidebar({ open, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      {open && <div className="fixed inset-0 top-16 bg-black/40 z-30 lg:hidden" onClick={onClose} />}
            ...
            <div key={group.group}>
              ...
              <ul className="space-y-0.5">
                {group.items.map((item) => {
                  const active = item.to === "/" ? currentPath === "/" : item.to && currentPath === item.to
                  return (
                    <li key={item.label}>
                      <button
                      ...
                      >
                        ...
                        {active && open && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-sky-500" />}
                      </button>
                    </li>
                  )
                })}
          ...
          <div className="p-3 border-t border-slate-100">
            ...
            >
              ...
              <div className="text-left">
                <p className="text-xs font-semibold text-slate-700">로그인이 필요해요</p>
